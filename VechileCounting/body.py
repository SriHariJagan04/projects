...
class Body(QWidget):
    ...
    #         print("Play ignored due to recent seek")
    #         return
    ...
    #     else:
    #         print("payTime:",self.format_time(self.media_player.get_time())) #LAST PLAY TIME
    #         self.media_player.play()
    #         self.play_btn.setText("⏸ Pause")
    #         self.is_paused = False
    ...
    def toggle_play(self):
        now = time.time()
        if now - self.last_seek_time < 0.3:
            print("⏸ Play ignored: too soon after seek")
            return
    ...
    def resume_after_seek(self):
        ...
        corrected_time = self.format_time(self.media_player.get_time())
        print("▶ Playing from:", corrected_time)
        self.log_action("▶ Playing from", corrected_time)

        headerTime = self.header.timestamps_input.text()
        self.log_action("▶ Playing from headerTime", headerTime)
    ...
    # def seek_backward(self):
    #     now = time.time()
    #     if now - self.last_seek_time < 0.5:
    #         return
    ...
    #     step = 500  #500 ms step to make it more noticeable and consistent
    #     current_time = self.media_player.get_time()
    #     new_time = max(current_time - step, 0)
    ...
    #     print("newTime",self.format_time(new_time)) #newTime stamps debug
    ...
    #     self.media_player.set_time(new_time)  # ✅ Set time immediately

    #     print("newTime", self.format_time(new_time))  # Debug
    ...
    def seek_backward(self):
        ...
        step = 500  # 500ms rewind
        current_time = self.media_player.get_time()
        new_time = max(current_time - step, 0)

        # Pause and apply seek immediately
        ...
        self.play_btn.setChecked(False)
        self.play_btn.setText("▶ Play")
        self.time_label.setText(f"⏪ {self.format_time(new_time)}")  # Show ⏪ as indicator

        # Disable play temporarily
        self.play_btn.setEnabled(False)
        QTimer.singleShot(300, lambda: self.play_btn.setEnabled(True))  # Re-enable after 0.3s

        rewind_time = self.format_time(new_time)
        print("Rewound to::", rewind_time)
        self.log_action("⏪ Rewound to", rewind_time)

        headerTime = self.header.timestamps_input.text()
        self.log_action("⏪ Rewound to HeaderTime", headerTime)
    ...
    def log_action(self, action_type, timestamp):
        try:
            with open("video_action_log.txt", "a", encoding="utf-8") as f:
                f.write(f"\n{action_type}: {timestamp}")

                # Only log diagnostic once per session
                if not getattr(self, "diagnostics_logged", False):
                    self.diagnostics_logged = True  # Mark as logged
                    ...
                    try:
                        ...
                        f.write(f"VLC Version: {version}\n")

                        if (major, minor, patch) < recommended:
                            f.write("⚠ VLC version is below recommended 3.0.16. May cause playback issues.\n")
                        else:
                            f.write("✅ VLC version is sufficient.\n")
                    except Exception as ve:
                        f.write(f"❌ VLC version info error: {ve}\n")

                    # System Info
                    try:
                        f.write(f"System: {platform.system()} {platform.release()} ({platform.machine()})\n")
                        f.write(f"CPU Load: {psutil.cpu_percent()}%\n")
                        f.write(f"Memory Usage: {psutil.virtual_memory().percent}%\n")
                    except Exception as se:
                        f.write(f"❌ System info error: {se}\n")
